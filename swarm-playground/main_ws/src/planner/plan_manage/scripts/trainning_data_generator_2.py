...
class DroneDataGenerator:
    ...
    def save_data(self):
        print("obstacle_cloud_seq", len(self.obstacle_cloud_seq), "data_seq", len(self.data_seq))
        obstacle_cloud_array = np.array(self.obstacle_cloud_seq, dtype=object)
        replay_data = np.array(self.data_seq, dtype=object)
        print("save data of shape, replay", replay_data.shape, "map", obstacle_cloud_array.shape)
